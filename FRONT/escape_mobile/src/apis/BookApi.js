...
export const getRegionCafeList = async (specificRegions, names, pages) => {
    console.log(specificRegions, names, pages)
    // let [_, specificRegion, name, page] = queryKey;
    const keyword = `${names}/${specificRegions}`;
    const RegionCafe = new URLSearchParams({
        region: `${names}/${specificRegions}`
    });
