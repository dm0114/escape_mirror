...
//    }
    ...
    public ResponseEntity<ReservationAdminDayDto> getReservationAdminDay(@RequestParam Date reservationDay, long storeId,HttpServletRequest httpServletRequest) {
        LOGGER.info("[AdministratorController] getReservationCount 호출");
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(new MediaType("application", "json", StandardCharsets.UTF_8));
